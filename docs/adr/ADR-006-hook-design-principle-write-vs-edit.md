...
- Layer-coverage reference: [`docs/architecture/etclovg-coverage.md`](../architecture/etclovg-coverage.md) — ETCLOVG 7-layer agent-harness taxonomy map. ADR-006 is the canonical artifact under the `G` (Governance) layer's hook-design discipline. The coverage map's `G — Strong` verdict cites this ADR.
