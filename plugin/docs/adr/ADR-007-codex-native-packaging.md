...
Amended by v3.4.1 on 2026-06-08: the root `plugin -> .` symlink was replaced by a real `plugin/` child package directory after Claude Code installs on Linux failed while materializing the self-symlink.
...
- A root-level `plugin/` directory mirrors the installable package payload because Codex marketplace entries require a child source path; `./plugin` is the stable child path.
