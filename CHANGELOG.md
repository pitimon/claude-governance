...
- Replaced the root `plugin -> .` symlink with a real `plugin/` child package directory so Claude Code installs on Linux do not fail while materializing the self-symlink.
- Kept Codex marketplace support intact by mirroring the installable plugin payload under `plugin/`, including `.codex-plugin/`, `skills/`, `docs/`, `hooks/`, `agents/`, `examples/`, and `scripts/`.
- Strengthened `tests/validate-plugin.sh` to reject a symlinked `plugin` path and verify the mirrored Codex child package stays in sync with the source files.
...
- Clean Claude Code install from `pitimon/claude-governance`.
- Codex marketplace smoke install from a temporary local marketplace.
